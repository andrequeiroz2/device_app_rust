...
//     pool: &PgPool,
//     device: DeviceCreate
// ) -> Result<DeviceCreateResponse, AppError>{
//
//     match sqlx::query_as!(
...
//         RETURNING
//         devices.uuid,
//         (SELECT uuid FROM users WHERE users.id = devices.user_id LIMIT 1) AS "user_uuid!: Uuid",
//         devices.name,
//         devices.device_type_int,
//         devices.device_type_text,
//         devices.border_type_int,
//         devices.border_type_text,
//         devices.device_condition_int,
//         devices.device_condition_text,
//         devices.mac_address,
//         devices.created_at,
//         devices.updated_at,
//         devices.deleted_at
...
//         device.get_user_id(),
//         device.get_name(),
//         device.get_device_type_int(),
//         device.get_device_type_text(),
//         device.get_border_type_int(),
//         device.get_border_type_text(),
//         device.get_device_condition_int(),
//         device.get_device_condition_text(),
//         device.get_mac_address()
//     ).fetch_one(pool)
//         .await{
//         Ok(device) => Ok(device),
//         Err(err) => Err(AppError::ConstraintViolation(
//             AppMsgError{
